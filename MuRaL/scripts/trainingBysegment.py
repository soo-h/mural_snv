...
def trainBySegment(config, args, checkpoint_dir=None):
    """
    Training funtion.
    ...
    elif segment_dataloader:
        cutoff_batch_size = 128
        dataloader_train = generate_data_batches_filt(dataset_train, config['sampled_segments'], cutoff_batch_size, shuffle=True, num_workers=segment_workers)
        dataloader_valid = generate_data_batches_filt(dataset_valid, config['sampled_segments'], cutoff_batch_size=0, shuffle=False, num_workers=segment_workers)
    
    else:
        print("Error: trainging by segment should use segment_dataloader, check input paramaters!")
        sys.exit()
    ...
    criterion = torch.nn.CrossEntropyLoss(reduction='sum')
    if mix_loss:
        combined_loss = loss_choice(mix_loss, criterion)
    ...
    def train_step(model, 
        ...
        # use grad accumulation strategy when samples in segment(batch) large than max_batch 
        if sample_num > max_batch:
            trainGradAccum(model, y, cont_x, cat_x, distal_x, max_batch)
        else:
            trainOneBatch(model, y, cont_x, cat_x, distal_x)
        
        return 0

    def trainGradAccum(model, y, cont_x, cat_x, distal_x, max_batch):
        optimizer.zero_grad()

        for i in range(0, y.shape[0], max_batch):
            curr_y = y[i: i+max_batch]
            curr_cont_x = cont_x[i: i+max_batch]
            curr_cat_x = cat_x[i: i+max_batch]
            curr_distal_x = distal_x[i: i+max_batch]

            if curr_y.shape[0] == 1:
                continue

            preds = model.forward((curr_cont_x, curr_cat_x), curr_distal_x)
            # tuple-- recode branch loss
            if isinstance(preds, tuple):
                branch_preds, preds = preds[:-1], preds[-1]
                # recode main loss in each branch
                branch_loss_recode(branch_preds, curr_y, criterion)

            # backward and recode loss
            if mix_loss:
                loss_combined, loss_main = combined_loss(preds, curr_y.long().squeeze())
                loss_combined.backward()
                mix_loss_recode(loss_combined, loss_main)
            else:
                loss = criterion(preds, curr_y.long().squeeze())
                loss.backward()
                total_loss_recode(loss)
    ...
    def trainOneBatch(model, y , cont_x, cat_x, distal_x):
        preds = model.forward((cont_x, cat_x), distal_x)

        # tuple-- recode branch loss
        if isinstance(preds, tuple):
            branch_preds, preds = preds[:-1], preds[-1]
            branch_loss_recode(branch_preds, y, criterion)

        if mix_loss:
            loss_combined, loss_main = combined_loss(preds, y.long().squeeze())
            loss_combined.backward()
            mix_loss_recode(loss_combined, loss_main)
        else:
    ...
    def total_loss_recode(loss):
        total_loss_tracker.add_loss(loss)
        training_losses.append(loss.item())

    def mix_loss_recode(loss_combined, loss_main):
        total_loss_tracker.add_loss(loss_main)
        combin_loss_tracker.add_loss(loss_combined.item())
        training_losses.append(loss_main.item())

    def branch_loss_recode(branch_preds, y, criterion):

        if len(branch_preds) == 2:
            preds_local, preds_distal = branch_preds
        ...
        elif len(branch_preds) == 3:
            preds_local, preds_distal, preds_distal2 = branch_preds
            loss_local = criterion(preds_local, y.long().squeeze())
            loss_distal = criterion(preds_distal, y.long().squeeze())
            ...
            print("Error: Only consider branch is 2 or 3.")
            sys.exit()
        return 0
    ...
    local_loss_tracker = LossTracker()
    distal_loss_tracker = LossTracker()
    distal_loss_tracker2 = LossTracker()
    total_loss_tracker = LossTracker()
    combin_loss_tracker = LossTracker()
    ...
    for epoch in range(epochs):
        ...
        distal_loss_tracker2.reset()
        total_loss_tracker.reset()
        combin_loss_tracker.reset()
        ...
        for y, cont_x, cat_x, distal_x in dataloader_train:
            ...
            # Forward Pass
            train_step(model, y, cont_x, cat_x, distal_x, device, max_batch=128)
            # time view
            if batch_count % 1000 == 0:
        ...
        with torch.no_grad():
            ...
            valid_preds= model_predict_m(model, dataloader_valid, criterion, device, n_class, distal=True)
            ...
            if mix_loss:
                print(f"Training Combin Loss from LossTracker: {combin_loss_tracker.report_total_losses()/train_size}")
            print(f"train_loss from LossTracker: {total_loss_tracker.report_total_losses()/train_size}")
            if local_loss_tracker.has_loss():
                print(f"Training Local Loss from LossTracker: {local_loss_tracker.report_total_losses()/train_size}")
            if distal_loss_tracker.has_loss():
                print(f"Training Distal Loss1 from LossTracker: {distal_loss_tracker.report_total_losses()/train_size}")
            if distal_loss_tracker2.has_loss():
...
def get_optimizer(optim: str,
    ...
    if optim == 'Adam':
        ...
        optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=learning_rate, weight_decay=weight_decay, amsgrad=True)

    elif optim == 'AdamW2':
        optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=learning_rate, weight_decay=weight_decay, amsgrad=True)
        
    elif optim == 'SGD':
        optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=learning_rate, weight_decay=weight_decay, momentum=0.98, nesterov=True)
     
    else:
        print('Error: unsupported optimization method', optim)
        sys.exit()
    return optimizer

def loss_choice(mix_loss, criterion):   

    criterion_auxiliary = nn.MSELoss()

    loss_config = {
        'criterion1': criterion,
        'criterion2': criterion_auxiliary
        }

    if mix_loss == 1:
        combined_loss = partial(combined_nomut_moments_loss, 
                                criterion1=criterion, 
                                criterion2=criterion_auxiliary)
    
    elif mix_loss == 2:
        combined_loss = partial(combined_moments_loss, 
                                criterion1=criterion, 
                                criterion2=criterion_auxiliary,
                                moment=2)
    elif mix_loss == 3:
        ...
        loss_config['moment'] = 4
        loss_config['nclass'] = 1
        combined_loss = partial(combined_moments_loss, 
                                **loss_config)
    else:
        print(f"mix_loss = {mix_loss} not find!")
        sys.exit()
