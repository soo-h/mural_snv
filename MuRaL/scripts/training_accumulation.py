...
def train_grad_accumu(config, args, checkpoint_dir=None):
    """
    Training funtion.
    ...
    if mix_loss:
        combined_loss = loss_choice(mix_loss)
    ...
    local_loss_tracker = LossTracker(number_batch_view)
    distal_loss_tracker = LossTracker(number_batch_view)
    distal_loss_tracker2 = LossTracker(number_batch_view)
    total_loss_tracker = LossTracker(number_batch_view)
        ...
        for y, cont_x, cat_x, distal_x in dataloader_train:
            ...
            if isinstance(preds, tuple):
                if len(preds) == 3:
                    ...
                    loss_local = criterion(preds_local, y.long().squeeze())  / accumulation_steps
                    loss_distal = criterion(preds_distal, y.long().squeeze()) / accumulation_steps
                    ...
                    loss_local = criterion(preds_local, y.long().squeeze()) / accumulation_steps
                    loss_distal = criterion(preds_distal, y.long().squeeze()) / accumulation_steps
                    loss_distal2 = criterion(preds_distal2, y.long().squeeze()) / accumulation_steps
                    local_loss_tracker.add_loss(loss_local)
                    distal_loss_tracker.add_loss(loss_distal)
            ...
            # loss compute
            if mix_loss:
                loss1 = criterion(preds, y.long().squeeze())
                loss, loss2 = combined_loss(preds, y.long().squeeze())
                total_loss1 += loss1.item()
                total_loss2 += loss2.item()
            else:
                loss = criterion(preds, y.long().squeeze()) / accumulation_steps
